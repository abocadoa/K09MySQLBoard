...
/*
 ...
 참조(Lookup)하는 API로 쉽게 말하면 외부에 있는 객체를 이름으로
 찾아오기 위한 기술이다.
...
public class DBConnPool {
	...
	/*
	 ...
	 추가해야한다.
	 자세한 내용은 교재 194페이지 참조.
	 */
	
	//기본 생성자
	public DBConnPool() {
		try {
			//1. Context객체를 저장한다.(Tomcat서버라고 생각하면 된다.)
			Context initCtx=new InitialContext();
			/*
			 2. 앞에서 생성한 객체를 통해 JNDI서비스 구조의 초기
			 	Root디렉토리를 얻어온다.
			 	여기서 얻어오는 톰캣의 루트디렉토리명은
			 	java:comp/env로 이미 정해져 있으므로 그대로 사용하면
			 	된다.
			 */
			...
			/*
			 3. server.xml에 등록한 네이밍을 lookup하여 DataSource를
			 	얻어온다.
			 	해당 데이터소스는 DB에 연결하기 위한 정보를 가지고있다.
			 */
			DataSource source=(DataSource)ctx.lookup("dbcp_mysql");
			//4. 커넥션풀에 생성해놓은 커넥션객체를 가져다가 사용한다.
