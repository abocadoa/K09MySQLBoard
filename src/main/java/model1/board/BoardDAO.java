...
public class BoardDAO extends JDBConnect{
	...
	public BoardDAO(ServletContext application) {
		//부모생성자 에서는 application을 통해 web.xml에 직접 접근하여
			//컨텍스트 초기화 파라미터를 얻어온다.
		super(application);
	}
	...
	//작성된 게시물을 반환한다.
		//특히 반환값은 여러개의 레코드를 반환할 수 있으므로
		//List컬렉션을 반환타입으로 정의한다.
	public List<BoardDTO> selectList(Map<String, Object> map){
		//List계열의 컬렉션을 생성한다.
			//이때 타입 매개변수는 BoardDTO객체로 설정한다.
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			/*
			 MySQL(MariaDB)의 경우 시퀀스가 별도로 없고 테이블 생성시 자동증가컬럼으로
			 지정하기 위한 auto_increment가 있다.
			 따라서 PK로 지정된 컬럼을 쿼리문에서 제외하면 된다.
			 */
			/*
			 오라클 사용시의 insert문.
			 시퀀스를 이용하여 일련번호를 입력한다.
		...
		try {
			...
			if(rs.next()) {
				...
				/*
				 ...
				 컬럼명 둘다 사용할 수 있다.
				 날짜인 경우에는 getDate()메소드로 추출할 수 있다.
				 */
				dto.setContent(rs.getString("content"));
				dto.setPostdate(rs.getDate("postdate"));
				dto.setId(rs.getString("id"));
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map){
		...
		try {
			...
			/*
			 ...
			 싱글쿼테이션이 자동으로 삽입된다.
			 여기서는 정수를 입력해야 하므로 setInt()를 사용하고, 인수로 전달되는 변수를
			 정수로 변환해야 한다.
			 */
			...
			rs=psmt.executeQuery();
			//결과레코드의 개수만큼 반복하여 List컬렉션에 저장한다.
			while(rs.next()) {
				BoardDTO dto=new BoardDTO();
				dto.setNum(rs.getString("num"));
				dto.setTitle(rs.getString("title"));
