...
public class ListController extends HttpServlet{
	...
	ServletException, IOException {
		...
		//Model(데이터베이스), View(JSP페이지)로 파라미터 및 데이터를 전달하기 위해
			//생성한다.
		...
		//전체 게시물의 개수를 카운트하여 반환한다.
			//게시물의 개수를 통해 가상번호를 부여하거나 전체 페이지수를 계산한다.
		int totalCount=dao.selectCount(map);
		
		//페이지 처리 start
		/*
		 서블릿에서 application내장객체를 얻어온 후 web.xml에 설정한 컨텍스트 초기화
		 파라미터를 읽어온다.
		 한페이지에 출력할 게시물의 개수와 한블럭당 출력할 페이지번호의 개수이다.
		 */
