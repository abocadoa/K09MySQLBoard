...
/*
 ...
 즉, 글쓰기 페이지만 보여주면 된다.
 내용을 모두 채운 후 작성(submit)할때는 post방식이므로 doPost()가 요청을 받아 처리한다.
 */
public class WriteController extends HttpServlet{
	...
	ServletException, IOException {
		...
		/*
		 파일업로드를 위해 MultipartRequest객체를 생성한다.
		 객체가 정상적으로 실행된다면 파일업로드는 완료된다.
		 */
		...
		/*
		 ...
		 만약 글 작성시 파일을 첨부하지 않더라도 객체는 생성되므로 폼값은 정상적으로 받을
		 수 있다.
		 */
		if(mr==null) {
			JSFunction.alertLocation(resp, "첨부파일이 제한 용량을 초과합니다.", 
					"../mvcboard/write.do");
			/*
			 객체가 생성되지 않아 업로드에 실패하면 나머지 폼값도 받을 수 없으므로 반드시
			 return을 통해 실행을 중지시켜야 한다.
			 */
			return;
		}
