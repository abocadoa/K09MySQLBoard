...
public class PassController extends HttpServlet{
	...
	ServletException, IOException {
		/*
		 /mvcboard/pass.do?mode=delete&idx=134
		...
		if(confirmed) {
			if(mode.equals("edit")) {
				/*
				 ...
				 session영역은 페이지를 이동해도 데이터가 공유되므로 해당 게시물을 수정완료
				 할때까지 유지하고, 수정이 완료되면 제거할 것이다.
				 여기에서 저장한 패스워드는 수정을 위한 update쿼리문의 where절에 사용할
				 예정이다.
				 */
				...
				/*
				 수정 페이지로 이동한다.
				 만약 아래 URL패턴을 이용해서 패스워드 검증없이 진입하였다면 세션에 저장된
				 패스워드가 없을것이므로 수정 처리가 되지 않도록 처리할 것이다.
				 */
