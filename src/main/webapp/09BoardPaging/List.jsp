...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
/* DB연결 CRUD작업을 위한 DAO객체를 생성한다. */
BoardDAO dao=new BoardDAO(application);

/* 
검색어가 있는 경우 클라이언트가 선택한 필드명과 검색어를 지정할
Map컬렉션을 생성한다.
*/
...
/* 
전체페이지수를 계산한다.
(전체게시물개수/페이지당출력할게시물개수)=>결과값의 올림처리
가령 게시물의 개수가 51개라면 나눴을때의 결과가 5.1이 된다.
...
/* 
목록에 처음 진입했을때는 페이지 관련 파라미터가 없는 상태이므로 무조건 1page로
지정한다.
만약 파라미터pageNum이 있다면 request내장객체를 통해 받아온 후 페이지번호로
지정한다.
...
/* 
게시물의 구간을 계산한다.
MySQL에서는 limit를 통해 게시물의 범위를 결정하므로 Oracle을 사용할때와 조금 다르게
구간을 계산해야한다.
형식] 쿼리문 ... limit 시작위치, 한페이지에출력할개수;
	한 페이지에 5개의 게시물을 출력한다 가정하면...
	start =>
		계산식 : (현재페이지-1) * 출력개수
			1페이지 : (1-1) * 5 = 0
			2페이지 : (2-1) * 5 = 5
	end =>
		한 페이지에 출력할 게시물의 개수인 pageSize를 그대로 사용하면 된다.
*/
int start=(pageNum-1)*pageSize;
/* 한페이지에 출력할 게시물의 개수를 설정한다. */
int end=pageSize;
...
<%
	...
	/* 출력할 게시물이 있는 경우에는 확장for문으로 List컬렉션에 저장된
	데이터의 개수만큼 반복하여 출력한다.*/
    int virtualNum = 0; 
    ...
    for (BoardDTO dto : boardLists)
    {
    	...
    	/* 전체게시물수-(((현재페이지-1)*한페이지출력개수)+countNum증가치) */
